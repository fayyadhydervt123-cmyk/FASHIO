...
def _validate_image(image):
    ...
    ALLOWED_IMAGE_TYPES = [
        ...
        "image/png",
        "image/webp",
    ]  # defines which image file types are allowed
...
def edit_category(request):
    if request.method == "POST":
        category_id = request.POST.get(
            "category_id", ""
        ).strip()  # Gets the category ID from the hidden input from submitted form
...
def view_products(request):
    ...
    active_variants_prefetch = Prefetch(
        "variants",
        queryset=ProductVariant.objects.filter(status="ACTIVE").prefetch_related("images"),
    ...
    products = (
        ...
        .annotate(total_stock=Sum("variants__stock", filter=Q(variants__status="ACTIVE")))
        .distinct()
        .order_by("-created_at")
    )
    ...
    for product in page_obj:
        ...
        product.display_status = (
            "ACTIVE" if (product.is_active and active_variants) else "INACTIVE"
        )
        first_variant = active_variants[0] if active_variants else None
        first_image = (
            first_variant.images.first() if first_variant and first_variant.images.all() else None
        )
...
def add_product(request):
    ...
    subcategories_json = {
        str(cat.id): [
            {"id": str(s.id), "name": s.name} for s in cat.subcategories.order_by("name")
        ]
...
def edit_subcategory(request):
    ...
    if (
        SubCategory.objects.filter(name__iexact=name, category=subcat.category)
        .exclude(id=subcat_id)
        .exists()
    ):
...
def delete_subcategory(request):
    if request.method == "POST":
        ...
        if subcat.products.exists():
            messages.error(
                request, f'Cannot delete "{subcat.name}" — it has products assigned to it.'
            )
            return redirect("subcategories")
...
def product_variants(request, product_id):
    ...
    product.total_stock = (
        variants.filter(status="ACTIVE").aggregate(total=Sum("stock"))["total"] or 0
    )
    product.display_status = (
        "ACTIVE" if product.is_active and variants.filter(status="ACTIVE").exists() else "INACTIVE"
    )

    return render(
        request,
        "products/product_variants.html",
        {
            "product": product,
            "variants": variants,
        },
...
def add_variant(request, product_id):
    ...
    product.total_stock = (
        product.variants.filter(status="ACTIVE").aggregate(total=Sum("stock"))["total"] or 0
    )
    ...
    if request.method == "POST":
        ...
        if ProductVariant.objects.filter(product=product, color__iexact=color, size=size).exists():
            errors["size"] = "This variant already exists."

        if errors:
            return render(
                request,
                "products/add_variant.html",
                {
                    "product": product,
...
def edit_variant(request, variant_id):
    ...
    if request.method == "POST":
        ...
        # ── Duplicate check ──────────────────────────────────
        if (
            ProductVariant.objects.filter(product=product, color__iexact=color, size=size)
...
def product_list(request):
    ...
    products = (
        Product.objects.filter(
            is_active=True, subcategory__isnull=False, subcategory__category__is_blocked=False
        )
    ...
    # Size filter
    if size:
        products = products.filter(variants__size=size, variants__status="ACTIVE").distinct()
    ...
    for product in products:
        ...
        for variant in active_variants:
            ...
            savings = variant.savings_display
            ...
            variant_data.append(
                {
                    ...
                    "effective_discount_percent": float(variant.effective_discount_percent),
                    "discounted_price": float(variant.discounted_price),
                    "savings_type": savings["type"],
                    "savings_amount": float(savings["amount"]),
    ...
    paginator = Paginator(product_list_data, 9)
...
def product_detail(request, product_id):
    ...
    for variant in variants:
        variant_images = [image.image.url for image in variant.images.all()]
...
def add_to_cart(request):
    ...
    if max_allowed <= 0:
        if is_ajax:
            return JsonResponse(
                {"success": False, "message": "This variant is out of stock."}, status=400
            )
    ...
    if quantity > max_allowed:
        if is_ajax:
            return JsonResponse(
                {"success": False, "message": f"Only {max_allowed} item(s) available."}, status=400
            )
    ...
    if not created:
        ...
        if new_quantity > max_allowed:
            ...
            messages.error(
                request, f"You can only add up to {max_allowed} item(s) for this variant."
            )
    ...
    removed_from_wishlist = (
        Wishlist.objects.filter(user=request.user, variant=variant).delete()[0] > 0
    )

    wishlist_notification_count = Wishlist.objects.filter(user=request.user).count()

    if is_ajax:
        message = "Product added to Shopping Bag."

        if removed_from_wishlist:
            message = "Product added to Shopping Bag and removed from Favorites."
    ...
    messages.success(request, "Product added to Shopping Bag.")
    return redirect(request.META.get("HTTP_REFERER", "product_list"))
...
def cart_page(request):
    ...
    cart_items = (
        ...
        .select_related(
            "product", "variant", "product__subcategory", "product__subcategory__category"
        )
    ...
    for item in cart_items:
        ...
        item.is_available = (
            ...
            and item.product.subcategory is not None
            and item.product.subcategory.category.is_blocked is False
            and item.variant.status == "ACTIVE"
            and item.variant.stock > 0
        )
    ...
    if applied_coupon_session:
        coupon, discount, error = validate_coupon(
            applied_coupon_session["code"], request.user, total_price
        )
        if error:
            request.session.pop("applied_coupon", None)
            request.session.modified = True
            messages.error(request, f"Coupon removed: {error}")
        else:
    ...
    available_coupons = get_available_coupons(request.user, total_price) if total_price > 0 else []
...
def update_cart_quantity(request):
    ...
    product_available = (
        cart_item.product.is_active
        and cart_item.product.subcategory is not None
        and cart_item.product.subcategory.category.is_blocked is False
        and cart_item.variant.status == "ACTIVE"
    )
    ...
    if action == "increase":
        if cart_item.quantity < max_allowed:
            ...
            if is_ajax:
                return JsonResponse(
                    {"success": False, "message": f"Maximum quantity is {max_allowed}."}
                )
    ...
    if is_ajax:
        try:
            ...
            subtotal = sum(
                (i.variant.discounted_price * i.quantity for i in cart_items), Decimal("0.00")
            )
            ...
            if applied_coupon_code:
                coupon = Coupon.objects.filter(code=applied_coupon_code, is_active=True).first()
                if coupon:
                    coupon_discount = coupon.apply_to(subtotal)
            ...
            response_data = {
                ...
                "final_total": totals["total_payable"],
            ...
            if not item_removed:
                ...
                response_data["item_subtotal"] = (
                    cart_item.variant.discounted_price * cart_item.quantity
                )
                response_data["max_quantity"] = max_allowed
...
def add_to_wishlist(request):
    ...
    if not variant_id:
        if is_ajax:
            return JsonResponse(
                {"success": False, "message": "Please select a product option."}, status=400
            )
    ...
    wishlist_item, created = Wishlist.objects.get_or_create(
        user=request.user, variant=variant, defaults={"product": variant.product}
    )
        ...
        return JsonResponse(
            {
                ...
                "message": message,
                "created": created,
                "wishlist_notification_count": request.session.get(
                    "wishlist_notification_count", 0
                ),
            }
        )
...
def add_wishlist_to_cart(request, item_id):
    ...
    product_available = (
        wishlist_item.product.is_active
        and wishlist_item.product.subcategory is not None
        and wishlist_item.product.subcategory.category.is_blocked is False
        and wishlist_item.variant.status == "ACTIVE"
        and wishlist_item.variant.stock > 0
    )
    ...
    cart_item, created = Cart.objects.get_or_create(
        ...
        defaults={"product": wishlist_item.product, "quantity": 1},
    )
...
def add_all_wishlist_to_cart(request):
    ...
    for item in wishlist_items:
        ...
        cart_item, created = Cart.objects.get_or_create(
            ...
            defaults={"product": item.product, "quantity": 1},
        )
